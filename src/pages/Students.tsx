...
export default function Students() {
  ...
  const filtered = (students as any[]).filter(s => (!filterClass || s.class_id === filterClass) && (!search || s.full_name.includes(search) || s.national_id.includes(search)));
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/20 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
          <div className="bg-card rounded-xl border shadow-xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()} dir="rtl">
            ...
            <div className="space-y-3">
              ...
              <input placeholder="رقم الهوية *" value={form.national_id} onChange={e => setForm({ ...form, national_id: e.target.value })} className="w-full px-4 py-2.5 bg-background border rounded-lg text-sm" />
            ...
            <div key={className}>
              ...
              <div className="bg-card rounded-lg border overflow-hidden">
                <table className="data-table">
                  <thead><tr><th>#</th><th>الطالب</th><th>رقم الهوية</th><th>إجراءات</th></tr></thead>
                  <tbody>
